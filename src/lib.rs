...
pub mod machine_id {
    ...
    /// Return machine id
    pub fn get_machine_id() -> Result<String, Box<dyn Error>> {
        let flags = winreg::enums::KEY_READ;
        if let Ok(id) = get_machine_guid_(flags) {
            return Ok(id);
        }
        
        let flags = winreg::enums::KEY_READ | winreg::enums::KEY_WOW64_32KEY;
        if let Ok(id) = get_machine_guid_(flags) {
            return Ok(id);
        }
        
        let flags = winreg::enums::KEY_READ | winreg::enums::KEY_WOW64_64KEY;
        if let Ok(id) = get_machine_guid_(flags) {
            return Ok(id);
        }
        return Err(String::from("get_machine_guid failed").into());
    }

    fn get_machine_guid_(flags : u32) -> Result<String, Box<dyn Error>> {
        let hklm = RegKey::predef(HKEY_LOCAL_MACHINE);
        if let Ok(crypto) = hklm.open_subkey_with_flags("SOFTWARE\\Microsoft\\Cryptography", flags) {
            if let Ok(id) = crypto.get_value::<String, &str>("MachineGuid") {
                return Ok(id.trim().to_string());
            }
        }
        return Err(format!("get_machine_guid(flags={}) failed",flags).into());
    }
}
